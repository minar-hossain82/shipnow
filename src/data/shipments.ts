...
export type ShipmentTableStatus = "Delivery" | "Completed" | "Pending";
...
export type Shipment = {
  ...
  tableStatus: ShipmentTableStatus;
  mark: string;
};

const baseShipments: Omit<Shipment, "tableStatus">[] = [
  ...
  { id:"#SH9457830", company:"AutoParts Pro", category:"Automotive", carrier:"Aramex", freight:"Road Freight", product:"Engine Components", weight:"1,680 kg", origin:"Detroit, MI", destination:"San Diego, CA", departure:"Mar 20, 2035 – 07:15 AM", arrival:"Mar 26, 2035 – 02:00 PM", progress:100, status:"Out for Delivery", mark:"✳" },
...
const tableProgressValues = [60, 75, 100, 40, 50, 90, 65, 80, 30, 85, 60] as const;

export const shipments: Shipment[] = Array.from({ length: 48 }, (_, index) => {
  const source = baseShipments[index % baseShipments.length];
  const status: ShipmentTableStatus = source.status === "Delivered" ? "Completed" : source.status === "Processing" ? "Pending" : "Delivery";
  const progress = tableProgressValues[index % tableProgressValues.length];
  if (index < baseShipments.length) return { ...source, progress, tableStatus: status };
  return { ...source, progress, tableStatus: status, id: `${source.id.slice(0, -2)}${String((Number(source.id.slice(-2)) + index) % 100).padStart(2, "0")}` };
});
