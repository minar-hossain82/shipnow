...
function NewestChevron({fixed=false}:{fixed?:boolean}){return <svg aria-hidden="true" className={`${styles.newestChevron} ${fixed?"w-4 h-4 shrink-0":""}`} viewBox="0 0 10 6" fill="none"><path d="m1 1 4 4 4-4" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round"/></svg>}
function CalendarIcon(){return <svg aria-hidden="true" className={`${styles.dateCalendarIcon} w-4 h-4 shrink-0`} viewBox="0 0 18 18" fill="none"><rect x="2.5" y="4" width="13" height="11.5" rx="1.5" stroke="currentColor" strokeWidth="1.35"/><path d="M5.5 2.5v3M12.5 2.5v3M2.5 7.25h13" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round"/></svg>}
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.tools}>
          ...
          {view==="table"?<div className={`${styles.menuWrap} ${styles.desktopDateControl}`} ref={dateRef}>
            <button type="button" className={`${styles.date} min-[768px]:max-[1024px]:inline-flex min-[768px]:max-[1024px]:items-center min-[768px]:max-[1024px]:gap-2 min-[768px]:max-[1024px]:whitespace-nowrap`} aria-haspopup="menu" aria-expanded={dateOpen} onClick={()=>{setDateOpen(value=>!value);setTableFilterOpen(false)}}><CalendarIcon/><span className="whitespace-nowrap">{dateRangeLabel}</span><NewestChevron fixed/></button>
